...
```{r, results='asis'}
cat("## ","Disruptions to Normal Splicing")
    
normaltable <- table[two_sd == TRUE & introns %in% sig_introns & SJ_IR == "SJ" & annotated == "Y"]
events <- normaltable[,.(seqnames,event,difference,probandpct,controlavg,frame_conserved)][order(abs(difference), decreasing = T)]
...
for(i in sig_introns){
    ...
    insigrow <- as.data.table(t(c("","non-significant events",insigrow,"")))
...
![`r testgenes`](normal_change_barplot.png)
