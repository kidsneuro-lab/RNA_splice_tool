...
```{r, echo=FALSE}
exons <- table[two_sd == TRUE & SJ_IR == "SJ" ]
introns <- table[two_sd == TRUE & SJ_IR == "IR" & normal == "Y"]
```

```{r, echo=FALSE}
table <- rbind(exons, introns)
table$seqnames <- paste0(table$seqnames,":",table$start,"-",table$end)
table$difference <- formattable::percent(table$difference)
table$probandpct <- formattable::percent(table$probandpct)
table$controlavg <- formattable::percent(table$controlavg)
```
...
for(row in seq(1,nrow(table))){
    unique <- if(length(table[row,unique]) == "") "" else "*"
...
sig_introns <- unique(table[abs(difference) > 0.05, introns])
...
for(i in sig_introns){
    ...
    introntable <- table[introns == i]
    
    events <- introntable[,.(seqnames,event,probandpct,controlavg,difference,frame_conserved)][order(abs(difference), decreasing = T)]
    
    names(events) <- c("chr:start-end","event","proband","controls","difference","frame")
